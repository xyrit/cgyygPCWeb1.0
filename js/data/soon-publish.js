...
//prizeList.length的长度不能用大于0这种判断了  数据结构变了 判断数组的长度已经是未定义了
...
$.ajax({
	type:'post',
	url:''+ajaxUrl+'/Home/Index/productWaitByDetail',
	dataType:'json',
	cache:false,
	data:{
		 pid:pid,
	...
	success: function(data){
		...
		/*2016-2-17 当有幸运码的时候隐藏倒计时，显示已经揭晓*********************************/  
		var lottery_code=parseInt(data['prize']['prizeList']['lottery_code']);
		if(lottery_code>0){
		   /*var nowIngStr='';
		   nowIngStr+='<div style="width:300px;height:50px;background-color:#feedd3;color:#333; margin:0 auto;font-size:18px;line-height:50px">幸运码已经揭晓</div>';
	       $("#clock").html(nowIngStr);*/
		   $("#clock").hide();
		   $(".result").show();
		   $("#lottery_code").html(startCode+lottery_code);
		   
		   /*****************start 填充是谁获得了幸运码，和参与的参与码**********************/
		      //中奖用户信息
			 ...
			 //传递全局变量，开奖码与查看参与码接口中的参与码对比，标红参与码中的开奖码
		      ...
		      $(".lucky-boy").hide(); 
		      $("#lucky-boy2").html(prizeStr);
		   /*****************end 填充是谁获得了幸运码，和参与的参与码**********************/
		...
		/*var pri_attend_count=prizeList[0]['attend_count'];//本次参与次数
		var pri_lottery_time=prizeList[0]['lottery_time'];//揭晓时间
		var pri_create_time=prizeList[0]['create_time'];//参与时间
		var pri_lucky_code=prizeList[0]['lucky_code'];//所获得的幸运码，以逗号隔开
		var pri_ip_address=prizeList[0]['ip_address'];//ip地址
		var prizeUser=data['prize']['prizeUser'];
		var pri_nickname=prizeUser[0]['nickname'];//用户昵称
		var pri_path=data['host']+prizeUser[0]['path'];//用户头像
		*/
		...
		var newDate_0 = new Date();
		newDate_0.setTime(time_0 * 1000);
		var pri_time_pri_lottery_time=newDate_0.format('yyyy-MM-dd h:m:s');
		//参与时间
		var  time_1= pri_create_time;
		var newDate_1 = new Date();
		newDate_1.setTime(time_1 * 1000);
		...
		for(var j = 0;j<s.length;j++){//在循环里面不能定义两个相同的变量，i和J 不能两个I
		  //把startCode分离出来，然后以分号连接成一个新数字
		   pri_trueAttendCode+=(startCode+s[j])+'，';
		}
		...
		var prizeStr='';
		var pri_luckyCodeStr='';
		prizeStr+='<h5>恭喜<span class="text-danger">'+pri_nickname+'</span>获得了本期商品</h5>';
		prizeStr+='<p>本期参与：<span class="text-danger">'+pri_attend_count+'</span>人次 <a href="javascript:;" data-toggle="modal" data-target="#myModal">点击查看>></a></p>';
        prizeStr+='<p>揭晓时间：<span>'+pri_time_pri_lottery_time+'</span></p>';           
        prizeStr+='<p>参与时间：<span>'+pri_time_pri_create_time+'</span></p>';     
		//弹出框中每次参与的幸运码
		pri_luckyCodeStr+='<div class="well nomargin">';
		pri_luckyCodeStr+='<p class="text-gray2">'+pri_time_pri_create_time+'</p>';//创建时间
		...
		$("#myAttend_count2").html("我一共参与了<span class='text-danger' style='padding:0 3px'>"+pri_attend_count+"</span>次");//弹出框中--我参与次数
		...
		*/
		...
		if(myLuckyCode.length==0){
		   $("#myAttend").html('您还没有参与本次夺宝哦！');
			   ...
			   for(i=0;i<myLuckyCode.length;i++){
				 // var attend_count=myLuckyCode[i]['attend_count'];
				  ...
				  my_luckyCodeStr+='<p class="text-gray2">'+my_create_time+'</p>';//创建时间
				  my_luckyCodeStr+='<p class="word-break">'+my_trueAttendCode+'</p>';//幸运码,弹出框中的
				  my_luckyCodeStr+='</div>'; */
				 
			   }
			   $("#myAttend").html('<p class="nomargin">本期参与：<span class="text-danger">'+myAttend_countCount+'</span>人次 <a href="javascript:;" class="text-blue" onClick="alertList('+userId+','+_lottery_id+');">点击查看>></a></p>');
		   ...
		   /***最新一期****/
		   if(data['newLottery'].length<0){
			   $("#newContent").html('<p class="text-center">暂无信息！</p>');
				...
				var new_lottery_id=data['newLottery'][0]['lottery_id'];
				 ...
				 newLotteryStr+='<a href="goods-details.html?pid='+goods_link+'" class="new-qishu-content"><h3>(第'+new_lottery_id+'期)'+new_title+'</h3></a>';
		   ...
		   if(prizeList.length!='undefined'){
		        ...
		        var hour_lottery=prizeList['hour_lottery'];//此字段在中奖用户json中，但不影响
				var pri_lottery_code=prizeList['lottery_code'];//开奖幸运码
				var hour_lottery_id=prizeList['hour_lottery_id'];//此字段在中奖用户json中，但不影响,时时彩开奖期号
		        var pri_last_attend_date_time=prizeList['last_attend_date_time'];//此字段在中奖用户json中，但不影响
			    
			   }
			   
			  
		   //alert(pri_last_attend_date_time);
		   if(pri_lottery_code==0){
			   $("#lottery_code2").html("？");
			   }
			   else{
		      $("#lottery_code2").html(startCode+pri_lottery_code);  //最终幸运号码
		   }
		  ...
		  $("#load_more").click(function(){
					...
					$(".glyphicon.glyphicon-plus").last().hide();//隐藏最后一个+号
					   ...
					   for(i=2;i<countList.length;i++){
						   ...
						   //时间,需要转为毫秒，现在只精确到秒，
							...
							var newDate_4 = new Date();
							newDate_4.setTime(time_4 * 1000);
							var result_create_time_4=newDate_4.format('yyyy-MM-dd hh:mm:ss.SS'); 
						   
						   resultStr2+='<tr>';
						   resultStr2+='<td class="text-center">'+create_date_time+'</td>';
		   ...
		   $("#pri_last_attend_time").html(pri_last_attend_date_time);//截止购买时间 最后参与时间
 		   /*************参与记录*************/
		   ...
		   /*****评论晒单***********/
			  ...
			  for(i=0;i<display.length;i++){
				 ...
				 /* var timestamp_5 = create_time;
				  ...
				  var newCreate_time_5=newDate_5.format('yyyy-MM-dd hh:mm:ss');*/
				 ...
				 // var picsid=display[i]['picsid'];//评论图片，以逗号隔开
                  ...
                  //disListStr+='<h4 class="media-heading">'+nickname+' <span class="text-gray">'+newCreate_time_5+'</span></h4>';
				...
				//获取用户的最后参与时间
				var pri_last_attend_time=data['prize']['prizeList']['last_attend_time'];
				
				var prizeUser=data['prize']['prizeUser'];
				if(prizeUser.length>0){
					//有中奖用户信息直接显示，不调用倒计时
					var pri_lottery_code_2=prizeList['lottery_code'];//开奖幸运码...在132行定义了 prizeList
				    $("#clock").hide();
				    $("#result_lucky").show();
					$("#lottery_code").html(pri_lottery_code_2);//获奖幸运码	
					
				}else{
					//没有中奖用户信息时调用倒计时
					/* start 如果最后一个用户购买时间大于或等于时时彩开奖时间，就用nextWaitTime*/
					  if(pri_last_attend_time>hourTime){
						
					  	waitTime_2=nextWaitTime;
					  }
					/* end */  
					  var intDiff = parseInt(waitTime_2);//倒计时总秒数量
					  ...
					  function timer(intDiff){
						  ...
						  if(intDiff<(-1)){
							   ...
							   //当倒计时为0时调用已经揭晓接口，获取幸运码，获取幸运用户信息，获取时时彩开奖号码
	  ...
	  success:function(data){
			  ...
			  if(data['code']!=200){
			   ...
			   for(var i=0;i<attendList.length;i++){//参与记录
					...
					var trueAttendLucky_code='';//把参与码分离出来，然后以分号连接成一个新数字
					var s=att_lucky_code.split(',');//把startCode分离出来，然后以分号连接成一个新数字
					for(var j = 0;j<s.length;j++){//在循环里面不能定义两个相同的变量，i和J 不能两个I
					   //trueAttendLucky_code+=(startCode+Number(s[j]))+'，';
						trueAttendLucky_code+='<span class="display-ib">'+(startCode+Number(s[j]))+'</span>'; 	 
...
//alert(waitTime_2);
/****************倒计时完了后调用的函数******************/
//var waitTime=waitTime_2;

function newLotteryGet(){
	
	$.ajax({
		type:'post',
		url:''+ajaxUrl+'/Home/Index/Onelottery',
		dataType:'json',
		cache:false,
		data:{
			 pid:pid,
		...
		success: function(data){
			...
			if(typeof(userInfo['uid'])!='undefined'){
				  ...
				  //倒计时完了后，隐藏当前倒计时层，打开幸运码层，隐藏，谁打开获奖幸运用户信息层
					   $("#clock").hide();
					   $(".result").show();
					   $("#who").empty();
					  //中奖用户信息
					  var pri_uid=userInfo['uid'];
					   var pri_nickname=userInfo['nickname'];
					   var pri_path=userInfo['path'];
					   var prizeCount=userInfo['prizeCount'];//用户参与的总数
					   var lottery_time=userInfo['lottery_time'];//揭晓时间
					   var lottery_code=parseInt(userInfo['lottery_code']);//开奖号码
						//开奖时间
					   ...
					   //时时彩、期号和中奖号码
					   var hour_lottery=userInfo['hour_lottery'];//时时彩开奖号码
					   var hour_lottery_id=userInfo['hour_lottery_id'];//时时彩开奖期号
					   var total_time=userInfo['total_time'];
					   var attend_ip=userInfo['attend_ip'];//用户参与IP 
					   var uid_link=pri_uid+'&'+'nickname='+escape(nickname);//当未登录或者查看他人的参与或者晒单或者中奖记录时 浏览器传递的参数（昵称需要编码传递）
					  // var last_attend_time=userInfo['last_attend_time'];//用户最后参与时间，传递查看计算详情
									  
					  //中奖用户信息
						var prizeStr='';
						
						if(typeof(userId) == "undefined"||pri_uid!=userId){
						   prizeStr+='<h5>恭喜<a href="personal-index-other.html?uid='+uid_link+'" class="text-danger" target="_blank">'+pri_nickname+'</a>获得了本期商品</h5>';
						   }else{
								prizeStr+='<h5>恭喜<a href="personal-index.html?per_market=2" class="text-danger" target="_blank">'+pri_nickname+'</a>获得了本期商品</h5>';
							   } 
						
						prizeStr+='<p>本期参与：<span class="text-danger">'+prizeCount+'</span>人次 <a href="javascript:;" class="text-blue" onClick="alertList('+pri_uid+','+lottery_id+');">点击查看>></a></p>';
						
						prizeStr+='<p>揭晓时间：<span>'+new_lottery_time+'</span></p>';
					   // prizeStr+='<p>参与时间：<span>'+pri_create_time+'</span></p>';     
						
						//判断中奖用户是否有头像
						 var p = picHostUrl(pic_host,pri_path);//此函数定义在common.js函数中
						 if(typeof(userId) == "undefined"||pri_uid!=userId){
								$("#lucky-img").html('<a href="personal-index-other.html?uid='+uid_link+'" target="_blank"><img src="'+p+'"></a>');
						 }else{
							 $("#lucky-img").html('<a href="personal-index.html?per_market=2" target="_blank"><img src="'+p+'"></a>');
							 }
					   ...
					   //中奖用户信息结束
						...
						//开奖号码
						$("#lottery_code").html(glo_startcode+lottery_code);
				        //传递全局变量，开奖码与查看参与码接口中的参与码对比，标红参与码中的开奖码
				        glo_ready_lotteryCode=glo_startcode+lottery_code;
					   
						$("#lottery_code2").html(glo_startcode+lottery_code);//获奖幸运码	---计算结果
						$("#hour-lottery").html(hour_lottery);//时时彩开奖号码  ---计算结果
						$("#hour-lottery_id").html(hour_lottery_id);//时时彩期号

				  
				
				}else{
					var nowIngStr='';
					nowIngStr+='<div style="width:300px;height:50px;background-color:#feedd3;color:#333; margin:0 auto;font-size:18px;line-height:50px">正在揭晓中...</div>';
					$("#clock").html(nowIngStr);
					//re_time();//重新加载
...
{  
  ...
  if(flag==1){
	 ...
	 timeout = true;
	  ...
	  setTimeout(re_time,30000); //time是指本身,延时递归调用自己,100为间隔调用时间,单位毫秒  
  }
  flag=1;
