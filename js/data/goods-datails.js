...
//定义全局变量
$("#lottery_id").html(lottery_id);//期号是传递过来的  直接使用
var index=0;
...
var glo_startcode;//弹出框中的开始码
//var glo_attendList;//定义参与记录的全局变量，在203行赋值

function getHref(lottery_id){
	$.ajax({  
	  type:'post',  
	  url:''+ajaxUrl+'/Home/Index/productByDetail',  
	  data:{
		 pid:pid,
		 uid:userId,
	     ...
	     var p_pageCount=data['p_pageCount']; 
		  ...
		  if(data['code']!=200){
		   ...
		   if(data['detail']['product'][0]['product_status']==1)//等待揭晓
		    {
			  window.location.href="soon-publish.html?pid="+pid+"&lottery_id="+lottery_id+"";
			}
		    else if(data['detail']['product'][0]['product_status']==0)//已经揭晓
			{
			  window.location.href="ready-publish.html?pid="+pid+"&lottery_id="+lottery_id+"";	
			}
		  ...
		  imgStr1+='</li>';
		   ...
		   glo_startcode=startCode;//弹出框中的开始码
		   var category=data['detail']['product'][0]['category'];//当前位置
		   $("#nowPosition").html(category);//当前位置
		   ...
		   for(var i=0;i<data['luckyCode'].length;i++){//幸运码
			   ...
			   /*var lucky_code=data['luckyCode'][i]['lucky_code'];
					 ...
					 // luckyCodeStr0+='<span>'+trueAttendCode+'</span>';//未弹出框前的参与码不需要显示
					  ...
					  luckyCodeStr+='<p class="word-break margin-r_8">'+trueAttendCode+'</p>';//幸运码,弹出框中的
					  luckyCodeStr+='</div>';*/
		      ...
		      // goodsTitle+=''+title+'';//标题
			   ...
			   if(data['attendList'].length==0){
				   attendList='<tr><td colspan="5" style="text-align:center">还没有参与记录哦，赶快来参与吧</td></tr>';
				      ...
				      //参与记录
					  //glo_attendList=data['attendList'];//赋值参与记录的全局；
					  ...
					  //var newCreate_timeAtt =$.myTime.UnixToDate(data['attendList'][i]['create_time'],true);//使用时间插件来初始化时间
				       ...
				       // var user_uid=uid+'&'+'rightContentMarking='+1;//给链接地址赋值,传递用户ID
						...
						//attendList+='<button  type="button" class="btn btn-default mypopover-btn noborder mypopover" title="本次参与'+data['attendList'][i]['attend_count']+'人次" data-container="body" data-content="'+trueAttendLucky_code+'" data-placement="left" data-toggle="popover"  data-html="true"> 查看幸运码</button>';
			  ...
			  /*进度条*****progressStr已经在上面定义有了*/
			   ...
			   if(myAttend_countCount<1){
				   $("#takein").html('<span class="top-triangle"></span><p class="nomargin text-center" style="font-size:14px;">您还没有参与本次夺宝哦！</p>');
					  ...
					  $("#alertList").attr("onClick","alertList("+userId+","+_lottery_id+");");
			  ...
			  // alert(data['detail'][0]['content']);
			   /*商品放大镜******等DOM结构完全加载才执行******/
			  //$("#goodsPic").html('<img src="'+imgSrc+'"/>');//占卜左边的商品图
			 
			  $(".jqzoom").imagezoom();//调用图片放大镜效果
			...
			//alert(data);
			  ...
			  if(data['code']!=200){
				...
				//for(i=0;i<prizeList.length;i++){
					...
					/*if(path==pic_host){
						   if(typeof(userId) == "undefined"||uid!=userId){
							  prizeListStr+='<div class="user-img"><a href="personal-index-other.html?uid='+user_uid+'" target="_blank"><img src="images/default1.png" width="100" alt="'+nickname+'"></a></div>';
							}else{
								prizeListStr+='<div class="user-img"><a href="personal-index.html?per_market=2" target="_blank"><img src="images/default1.png" width="100" alt="'+nickname+'"></a></div>';
							}
						
					 }else{
						prizeListStr+='<div class="user-img"><a href="personal-index-other.html?uid='+user_uid+'" target="_blank"><img src="'+path+'" width="100" alt="'+nickname+'"></a></div>';
					 } */    
					 var p = picHostUrl(pic_host,path);//此函数定义在common.js函数中
					 if(typeof(userId) == "undefined"||uid!=userId){
						prizeListStr+='<div class="user-img"><a href="personal-index-other.html?uid='+user_uid+'" target="_blank"><img src="'+p+'" width="100" alt="'+nickname+'"></a></div>';
					...
					if(typeof(userId) == "undefined"||uid!=userId){
						prizeListStr+='<a href="personal-index-other.html?uid='+user_uid+'" class="link-blue" target="_blank">'+nickname+'</a>'; 
							...
							prizeListStr+='<a href="personal-index.html?per_market=2" target="_blank" class="link-blue" target="_blank">'+nickname+'</a>'; 
							}
	  ...
	  success:function(data){
			  ...
			  if(data['code']!=200){
			   ...
			   for(var i=0;i<data['attendList'].length;i++){//参与记录
					...
					//attendList+='<button  type="button" class="btn btn-default mypopover-btn noborder mypopover" title="本次参与'+attend_countList+'人次" data-content="'+trueAttendLucky_code+'" data-trigger="focus" data-placement="left" data-toggle="popover"> 查看幸运码</button>';
...
var att_num=$("#count-input").val();
 //输入数字后，input失去焦点后重新赋值（参与数量给立即参与和加入购物车按钮）
  ...
  $("#count-input").blur(function(att_num){
	  ...
	  $("#nowAdd").attr("onClick","goToCar_two("+lottery_id+","+pid+","+att_num+");");//立即参与，必须在此，异步，等DOM建立之后
	  $("#nowAdd2").attr("onClick","goToCar_two("+lottery_id+","+pid+","+att_num+");");//立即参与，必须在此，异步，等DOM建立之后--占卜的
...
function add(att_num){
    ...
    $("#nowAdd").attr("onClick","goToCar_two("+lottery_id+","+pid+","+att_num+");");
	$("#nowAdd2").attr("onClick","goToCar_two("+lottery_id+","+pid+","+att_num+");");//占卜的
	$("#addTocarBtn").attr("onClick","addToCar_two("+lottery_id+","+pid+","+att_num+");");//加入购物车按钮
...
function red(att_num) {
	...
	$("#nowAdd").attr("onClick", "goToCar_two(" + lottery_id + "," + pid + "," + att_num + ");");//立即参与
	$("#nowAdd2").attr("onClick", "goToCar_two(" + lottery_id + "," + pid + "," + att_num + ");");//立即参与，占卜的
	$("#addTocarBtn").attr("onClick", "addToCar_two(" + lottery_id + "," + pid + "," + att_num + ");");//加入购物车按钮
//点击加入购物车按钮时显示+1
	$("#addTocarBtn").click(function () {
		if (typeof(user_token) != "undefined") {//查看参与弹出框------调用新接口
