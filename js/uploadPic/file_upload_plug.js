...
UploadAssist.prototype = {
    ...
    createInputFile: function () {
		...
		//var input2= dcmt.createElement("input");
		...
		/*$(input).attr("name", "input2");
		$(input).attr("name", "input3");*/
		$(input).attr("id", input.name);	
		
       //$(input2).attr("value","提交");
           ...
           // alert(this.name);
            ...
            if (that.settings.perviewElementId) {
                if (!that.settings.perviewImgStyle) {
                    ...
                    if ('undefined' == typeof(document.body.style.maxHeight)&&!$.support.leadingWhitespace) {
